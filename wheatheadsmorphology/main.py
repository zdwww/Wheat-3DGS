...
from pydantic import BaseModel, DirectoryPath, StrictInt, StrictFloat, conint, confloat, field_validator
...
class ConfigModel(BaseModel):
    ...
    def validate_paths(cls, raw_cfg: dict, base_dir: Path) -> dict:
        ...
        if 'data_folder' in raw_cfg:
            expanded = expandvars(str(raw_cfg['data_folder']))
            resolved['data_folder'] = (base_dir / expanded).resolve()
        if 'output_folder' in raw_cfg:
            expanded_out = expandvars(str(raw_cfg['output_folder']))
            resolved['output_folder'] = (base_dir / expanded_out).resolve()
        return resolved

# Allowing running custom config.yaml using CLI
def get_args():
    p = argparse.ArgumentParser(description="Run the full pipeline with a given config YAML file")
    p.add_argument(
        "-c", "--config",
        type=Path,
